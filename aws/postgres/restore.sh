...
docker run --rm --network host -e PGPASSWORD=$TARGET_DATABASE_PASSWORD -v "$PWD:/backup" postgres:16 psql -h localhost -p $TARGET_DATABASE_PORT -U $TARGET_DATABASE_USERNAME -d $TARGET_DATABASE_NAME --file=/backup/$BACKUP_FILE_NAME
